...
public class CalendarRepositoryTests
{
    ...
    public async TaskSystem GetInterviewsAsync_FiltersByUserAndDateRangeAndSorts()
    {
        ...
        var archivedApplication = new Application { Id = 3, UserId = 1, CompanyId = 1, Company = company, JobTitle = "Archived Developer", Status = Status.Gesprek, IsArchived = true, ArchivedAt = DateTime.UtcNow, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow };

        context.Companies.AddRange(company, otherCompany);
        context.Applications.AddRange(application, otherApplication, archivedApplication);
        context.Interviews.AddRange(
            ...
            new Interview { Id = 5, ApplicationId = 3, Application = archivedApplication, InterviewType = "Online", ScheduledStart = new DateTime(2026, 4, 15, 9, 0, 0), CreatedAt = DateTime.UtcNow }
        );
